	...
	static <T> T callSync(RestApiRequest<T> request) {
		try {
			...
			if (response != null && response.body() != null) {
				...
				throw new HuobiApiException(HuobiApiException.ENV_ERROR,
			...
			throw new HuobiApiException(HuobiApiException.ENV_ERROR, "[Invoking] Unexpected error: " + e.getMessage());
		}
	}
			...
			call.enqueue(new Callback() {
				...
				public void onFailure(Call call, IOException e) {
					FailedAsyncResult<T> result = new FailedAsyncResult<>(
							new HuobiApiException(HuobiApiException.RUNTIME_ERROR, "[Invoking] Rest api call failed"));
					try {
				...
				public void onResponse(Call call, Response response) {
					...
					try {
						...
						FailedAsyncResult<T> result = new FailedAsyncResult<>(new HuobiApiException(
