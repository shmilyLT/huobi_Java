...
class InputChecker {
	...
	<T> InputChecker shouldNotNull(T value, String name) {
		if (value == null) {
			throw new HuobiApiException(HuobiApiException.INPUT_ERROR, "[Input] " + name + " should not be null");
		}
		return checkerInst;
	}
	...
	<T> InputChecker shouldNull(T value, String name) {
		if (value != null) {
			throw new HuobiApiException(HuobiApiException.INPUT_ERROR, "[Input] " + name + " should be null");
		}
		return checkerInst;
	}
	...
	InputChecker checkSymbol(String symbol) {
		...
		if (isSpecialChar(symbol)) {
			throw new HuobiApiException(HuobiApiException.INPUT_ERROR, "[Input] " + symbol + " is invalid symbol");
		}
		return checkerInst;
	}
	...
	InputChecker checkCurrency(String currency) {
		if (currency == null || "".equals(currency)) {
			throw new HuobiApiException(HuobiApiException.INPUT_ERROR, "[Input] Currency is mandatory");
		}
		if (isSpecialChar(currency)) {
			throw new HuobiApiException(HuobiApiException.INPUT_ERROR, "[Input] " + currency + " is invalid currency");
		}
		return checkerInst;
	}
	...
	InputChecker checkETF(String symbol) {
		if (!"hb10".equals(symbol)) {
			throw new HuobiApiException(HuobiApiException.INPUT_ERROR, "currently only support hb10 :-)");
		}
		return checkerInst;
	}
