...
class RestApiRequestImpl {
	...
	RestApiRequest<List<Candlestick>> getCandlestick(String symbol, CandlestickInterval interval, Long startTime,
		...
		InputChecker.checker().checkSymbol(symbol).checkRange(size, 1, 2000, "size").shouldNotNull(interval,
				"CandlestickInterval");
		RestApiRequest<List<Candlestick>> request = new RestApiRequest<>();
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("symbol", symbol).putToUrl("period", interval)
				.putToUrl("size", size).putToUrl("start", startTime).putToUrl("end", endTime);
	...
	RestApiRequest<PriceDepth> getPriceDepth(String symbol, Integer size) {
		...
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("symbol", symbol).putToUrl("type", "step0");
	...
	RestApiRequest<List<Trade>> getHistoricalTrade(String symbol, String fromId, Integer size) {
		...
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("symbol", symbol).putToUrl("size", size); // .put("fromid",
																												// fromId)
	...
	RestApiRequest<TradeStatistics> get24HTradeStatistics(String symbol) {
		...
		request.jsonParser = (jsonWrapper -> {
			...
			tradeStatistics.setTimestamp(TimeService.convertCSTInMillisecondToUTC(jsonWrapper.getLong("ts")));
			JsonWrapper tick = jsonWrapper.getJsonObject("tick");
	...
	RestApiRequest<List<Withdraw>> getWithdrawHistory(String currency, Long fromId, Integer size) {
		InputChecker.checker().checkCurrency(currency).shouldNotNull(fromId, "fromTd").shouldNotNull(size, "size");
		RestApiRequest<List<Withdraw>> request = new RestApiRequest<>();
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("currency", currency).putToUrl("type", "withdraw")
				.putToUrl("from", fromId).putToUrl("size", size);
		...
		request.jsonParser = (jsonWrapper -> {
			...
			dataArray.forEach((item) -> {
				...
				withdraw.setCreatedTimestamp(TimeService.convertCSTInMillisecondToUTC(item.getLong("created-at")));
				withdraw.setUpdatedTimestamp(TimeService.convertCSTInMillisecondToUTC(item.getLong("updated-at")));
	...
	RestApiRequest<List<Deposit>> getDepositHistory(String currency, Long fromId, Integer size) {
		...
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("currency", currency).putToUrl("type", "deposit")
				.putToUrl("from", fromId).putToUrl("size", size);
		request.request = createRequestByGetWithSignature("/v1/query/deposit-withdraw", builder);
		request.jsonParser = (jsonWrapper -> {
			...
			dataArray.forEach((item) -> {
				...
				deposit.setCreatedTimestamp(TimeService.convertCSTInMillisecondToUTC(item.getLong("created-at")));
				deposit.setUpdatedTimestamp(TimeService.convertCSTInMillisecondToUTC(item.getLong("updated-at")));
	...
	RestApiRequest<Long> transfer(TransferRequest transferRequest) {
		InputChecker.checker().checkSymbol(transferRequest.symbol).checkCurrency(transferRequest.currency)
				.shouldNotNull(transferRequest.from, "from").shouldNotNull(transferRequest.to, "to")
		...
		} else if (transferRequest.from == AccountType.MARGIN && transferRequest.to == AccountType.SPOT) {
			address = "/v1/dw/transfer-out/margin";
		} else {
			throw new HuobiApiException(HuobiApiException.INPUT_ERROR, "[Input] incorrect transfer type");
		}
		RestApiRequest<Long> request = new RestApiRequest<>();
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToPost("currency", transferRequest.currency)
				.putToPost("symbol", transferRequest.symbol).putToPost("amount", transferRequest.amount);
	...
	RestApiRequest<Long> applyLoan(String symbol, String currency, BigDecimal amount) {
		InputChecker.checker().checkSymbol(symbol).checkCurrency(currency).shouldNotNull(amount, "amount");
		RestApiRequest<Long> request = new RestApiRequest<>();
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToPost("currency", currency).putToPost("symbol", symbol)
	...
	RestApiRequest<List<Loan>> getLoan(LoanOrderRequest loanOrderRequest) {
		...
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("symbol", loanOrderRequest.getSymbol())
				.putToUrl("start-date", loanOrderRequest.getStartDate(), "yyyy-MM-dd")
				.putToUrl("end-date", loanOrderRequest.getEndDate(), "yyyy-MM-dd")
				.putToUrl("states", loanOrderRequest.getStates()).putToUrl("from", loanOrderRequest.getFromId())
	...
	RestApiRequest<Long> createOrder(NewOrderRequest newOrderRequest) {
		InputChecker.checker().checkSymbol(newOrderRequest.getSymbol())
				...
				.shouldNotNull(newOrderRequest.getAmount(), "Amount").shouldNotNull(newOrderRequest.getType(), "Type");

		if (newOrderRequest.getType() == OrderType.SELL_LIMIT || newOrderRequest.getType() == OrderType.BUY_LIMIT
		...
		if (newOrderRequest.getType() == OrderType.SELL_MARKET || newOrderRequest.getType() == OrderType.BUY_MARKET) {
		...
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToPost("account-id", account.getId())
				.putToPost("amount", newOrderRequest.getAmount()).putToPost("price", newOrderRequest.getPrice())
				.putToPost("symbol", newOrderRequest.getSymbol()).putToPost("type", newOrderRequest.getType())
	...
	RestApiRequest<List<Order>> getOpenOrders(OpenOrderRequest openOrderRequest) {
		...
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("account-id", account.getId())
				.putToUrl("symbol", openOrderRequest.getSymbol()).putToUrl("side", openOrderRequest.getSide())
	...
	RestApiRequest<Void> cancelOrders(String symbol, List<Long> orderIds) {
		InputChecker.checker().checkSymbol(symbol).shouldNotNull(orderIds, "orderIds").checkList(orderIds, 1, 50,
				"orderIds");
		RestApiRequest<Void> request = new RestApiRequest<>();
		List<String> stringList = new LinkedList<>();
	...
	RestApiRequest<BatchCancelResult> cancelOpenOrders(CancelOpenOrderRequest cancelOpenOrderRequest) {
		InputChecker.checker().checkSymbol(cancelOpenOrderRequest.getSymbol())
				.shouldNotNull(cancelOpenOrderRequest.getAccountType(), "AccountType");
		Account account = AccountsInfoMap.getUser(apiKey).getAccount(cancelOpenOrderRequest.getAccountType());
		if (account == null) {
			throw new HuobiApiException(HuobiApiException.INPUT_ERROR, "[Input] No such account");
		}
		...
		request.request = createRequestByPostWithSignature("/v1/order/orders/batchCancelOpenOrders", builder);
		request.jsonParser = (jsonWrapper -> {
	...
	RestApiRequest<Order> getOrder(String symbol, long orderId) {
		...
		request.jsonParser = (jsonWrapper -> {
			...
			order.setCanceledTimestamp(TimeService.convertCSTInMillisecondToUTC(data.getLong("canceled-at")));
			order.setCreatedTimestamp(TimeService.convertCSTInMillisecondToUTC(data.getLong("created-at")));
			order.setFinishedTimestamp(TimeService.convertCSTInMillisecondToUTC(data.getLong("finished-at")));
	...
	RestApiRequest<List<MatchResult>> getMatchResults(String symbol, long orderId) {
		...
		request.jsonParser = (jsonWrapper -> {
			...
			dataArray.forEach((item) -> {
				...
				matchResult.setCreatedTimestamp(TimeService.convertCSTInMillisecondToUTC(item.getLong("created-at")));
	...
	RestApiRequest<List<MatchResult>> getMatchResults(MatchResultRequest matchResultRequest) {
		InputChecker.checker().checkSymbol(matchResultRequest.getSymbol()).checkRange(matchResultRequest.getSize(), 1,
				100, "size");
		RestApiRequest<List<MatchResult>> request = new RestApiRequest<>();
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("symbol", matchResultRequest.getSymbol())
	...
	RestApiRequest<Long> withdraw(WithdrawRequest withdrawRequest) {
		InputChecker.checker().checkCurrency(withdrawRequest.getCurrency())
				.shouldNotNull(withdrawRequest.getAddress(), "address")
				.shouldNotNull(withdrawRequest.getAmount(), "amount");
		RestApiRequest<Long> request = new RestApiRequest<>();
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToPost("address", withdrawRequest.getAddress())
				.putToPost("amount", withdrawRequest.getAmount()).putToPost("currency", withdrawRequest.getCurrency())
				.putToPost("fee", withdrawRequest.getFee()).putToPost("addr-tag", withdrawRequest.getAddressTag());
	...
	RestApiRequest<List<Order>> getHistoricalOrders(HistoricalOrdersRequest req) {
		InputChecker.checker().checkSymbol(req.getSymbol()).shouldNotNull(req.getState(), "state");
		RestApiRequest<List<Order>> request = new RestApiRequest<>();
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("symbol", req.getSymbol())
				.putToUrl("types", req.getType()).putToUrl("start-date", req.getStartDate(), "yyyy-MM-dd")
				.putToUrl("end-date", req.getEndDate(), "yyyy-MM-dd").putToUrl("from", req.getStartId())
				.putToUrl("states", req.getState()).putToUrl("size", req.getSize());
	...
	RestApiRequest<Long> transferBetweenParentAndSub(TransferMasterRequest req) {
		InputChecker.checker().checkCurrency(req.getCurrency()).shouldNotNull(req.getAmount(), "amount")
				.shouldNotNull(req.getSubUid(), "sub-uid").shouldNotNull(req.getType(), "type");
		RestApiRequest<Long> request = new RestApiRequest<>();
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToPost("sub-uid", req.getSubUid())
				.putToPost("amount", req.getAmount()).putToPost("currency", req.getCurrency())
	...
	RestApiRequest<List<Candlestick>> getETFCandlestick(String symbol, CandlestickInterval interval, Integer size) {
		InputChecker.checker().checkETF(symbol).checkRange(size, 1, 2000, "size").shouldNotNull(interval, "interval");
	...
	RestApiRequest<Void> etfSwap(String etfSymbol, int amount, EtfSwapType swapType) {
		...
		RestApiRequest<Void> request = new RestApiRequest<>();
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToPost("etf_name", etfSymbol).putToPost("amount",
				amount);
	...
	RestApiRequest<List<EtfSwapHistory>> getEtfSwapHistory(String etfSymbol, int offset, int size) {
		InputChecker.checker().checkSymbol(etfSymbol).checkRange(size, 1, 100, "size").greaterOrEqual(offset, 0,
				"offset");
		RestApiRequest<List<EtfSwapHistory>> request = new RestApiRequest<>();
		UrlParamsBuilder builder = UrlParamsBuilder.build().putToUrl("etf_name", etfSymbol).putToUrl("offset", offset)
				.putToUrl("limit", size);
