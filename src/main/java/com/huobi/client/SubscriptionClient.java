...
public interface SubscriptionClient {
	 ...
	 *
	 * @param symbols  The symbols, like "btcusdt". Use comma to separate multi
	 *                 symbols, like "btcusdt,ethusdt".
	 * @param interval The candlestick/kline interval, MIN1, MIN5, DAY1 etc.
	 * @param callback The implementation is required. onReceive will be called if
	 *                 receive server's update.
	 */
	...
	/**
	 * Subscribe price depth event. If the price depth is updated, server will send
	 * the data to client and onReceive in callback will be called.
	 *
	 * @param symbols  The symbols, like "btcusdt". Use comma to separate multi
	 *                 symbols, like "btcusdt,ethusdt".
	 * @param callback The implementation is required. onReceive will be called if
	...
	/**
	 * Subscribe account changing event. If the balance is updated, server will send
	 * the data to client and onReceive in callback will be called. default to
	 * subscribe the available balance.
	 *
	 ...
	 * the data to client and onReceive in callback will be called.
	 *
	 * @param mode         when mode is AVAILABLE, balance refers to available
	 *                     balance; when mode is TOTAL, balance refers to TOTAL
	...
	/**
	 * Subscribe order changing event. If a order is created, canceled etc, server
	...
	/**
	 * Subscribe 24 hours trade statistics event. If statistics is generated, server
	 * will send the data to client and onReceive in callback will be called.
	 *
	 * @param symbols  The symbols, like "btcusdt". Use comma to separate multi
	 *                 symbols, like "btcusdt,ethusdt".
	 ...
	 *                 receive server's update.
	 */
	void subscribe24HTradeStatisticsEvent(String symbols, SubscriptionListener<TradeStatisticsEvent> callback);
	...
	static SubscriptionClient create(String apiKey, String secretKey) {
		return HuobiApiInternalFactory.getInstance().createSubscriptionClient(apiKey, secretKey,
	...
	static SubscriptionClient create(String apiKey, String secretKey, SubscriptionOptions subscriptionOptions) {
		return HuobiApiInternalFactory.getInstance().createSubscriptionClient(apiKey, secretKey, subscriptionOptions);
	}
